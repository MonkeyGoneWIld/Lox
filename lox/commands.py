...
from lox.tagger.audio_info import gather_audio_info
from lox.tagger.combine import combine_metadatas
from lox.tagger.metadata import clean_metadata, remove_various_artists
from lox.tagger.retagger import create_artist_str
from lox.tagger.sources import run_metadata
...
@click.option(
    "--tracker",
    "-t",
    help=f"Tracker choices: ({'/'.join(lox.trackers.tracker_list)})",
)
@click.argument(
    "path",
...
async def checkspecs(tracker: str | None, torrent_id: str | None, path: str) -> None:
    ...
    if "/torrents.php" in torrent_id_input:
        ...
        if base_url in lox.trackers.tracker_url_code_map:
            # this will overide -t tracker
            tracker = lox.trackers.tracker_url_code_map[base_url]
        else:
    ...
    tracker = await lox.trackers.validate_tracker(None, "tracker", tracker)
...
@click.option(
    "--tracker",
    "-t",
    type=click.Choice(lox.trackers.tracker_list, case_sensitive=False),
    help=f"Choices: ({'/'.join(lox.trackers.tracker_list)})",
)
@click.option(
    "--metadata",
    "-m",
...
async def checkconf(tracker: str | None, metadata: bool, seedbox: bool, reset: bool) -> None:
    ...
    if not (metadata or seedbox) or tracker:
        trackers = [tracker] if tracker else lox.trackers.tracker_list

        for t in trackers:
            click.secho(f"\n[ Testing Tracker: {t} ]", fg="cyan", bold=True)

            try:
                lox.trackers.get_class(t)()
