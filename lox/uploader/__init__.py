...
import lox.trackers
from lox import cfg
from lox.checks import mqa_test
from lox.checks.integrity import (
    check_integrity,
    format_integrity,
    sanitize_integrity,
)
...
from lox.converter.transcoding import (
    generate_transcode_description,
    transcode_folder,
)
from lox.errors import AbortAndDeleteFolder, CRCMismatchError, EditedLogError, InvalidMetadataError
from lox.images import upload_cover
from lox.tagger import (
    metadata_validator_base,
    validate_encoding,
    validate_source,
)
from lox.tagger.audio_info import (
    check_hybrid,
    gather_audio_info,
    recompress_path,
)
from lox.tagger.cover import compress_pictures, download_cover_if_nonexistent
from lox.tagger.foldername import rename_folder
from lox.tagger.folderstructure import check_folder_structure
from lox.tagger.metadata import get_metadata
from lox.tagger.pre_data import construct_rls_data
from lox.tagger.retagger import rename_files, tag_files
from lox.tagger.review import review_metadata
from lox.tagger.tags import check_tags, gather_tags, standardize_tags
from lox.uploader.dupe_checker import (
...
from lox.uploader.preassumptions import confirm_group_upload, print_preassumptions
from lox.uploader.request_fill import check_requests
from lox.uploader.seedbox import UploadManager
...
from lox.uploader.upload import (
    concat_track_data,
    prepare_and_upload,
)
...
@click.option(
    "--tracker",
    "-t",
    callback=lox.trackers.validate_tracker,
    help=f"Uploading Choices: ({'/'.join(lox.trackers.tracker_list)})",
)
...
async def up(
    ...
    if request:
        request = lox.trackers.validate_request(gazelle_site, request)
...
async def upload_and_report(
    ...
    if cfg.upload.upload_to_seedbox:
        ...
        tracker_code = gazelle_site.site_code
        seedbox_uploader.add_upload_task(path, task_type="folder", is_flac=is_flac, tracker=tracker_code)
        seedbox_uploader.add_upload_task(torrent_path, task_type="seed", is_flac=is_flac, tracker=tracker_code)
